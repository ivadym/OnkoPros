...
function generarJWT(req, res, next) {
  logger.info('ID usuario: ' + req.usuario.IdUsuario + ' > jwtController.generarJWT');
  try {
    ...
    logger.error('ID usuario: ' + req.usuario.IdUsuario + ' > jwtController.generarJWT.500');
...
function verificarJWT(req, res, next) {
  logger.info('ID usuario: ' + req.headers['id'] + ' > jwtController.verificarJWT');
  try {
    ...
    jwt.verify(token, publicKey, function(err, decoded) {
      if (err || !decoded) {
        logger.error('ID usuario: ' + req.headers['id'] + ' > jwtController.verificarJWT.403');
        ...
        logger.error('ID usuario: ' + req.headers['id'] + ' > jwtController.verificarJWT.403.1');
        var err = new Error('Forbidden');
        err.statusCode = 403; // HTTP 403 Forbidden
        next(err);
      }
    ...
    logger.error('ID usuario: ' + req.headers['id'] + ' > jwtController.verificarJWT.500');
    var err = new Error(error.message ? error.message : error);
    err.statusCode = 500; // HTTP 500 Internal Server Error
    next(err);
  }
